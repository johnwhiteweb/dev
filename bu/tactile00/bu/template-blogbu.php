          ...
          <!--<div class="title"><h2>All</h2><a href=""><span class="see-all">All</span></a><hr></div>-->
			...
			?>
				...
				<div class="text-box">
				  ...
				  <span class="date"><?php echo date('M d, Y', strtotime($item->post_date)); ?></span>
				  <?php 
				//	$content= $item->post_excerpt;//	$content=strpos($item->post_content, ' ', 100);	//	$content = strip_tags($item->post_content);
				//	$content = substr($content, 0, 100);// if(empty($content)){	// $content = strip_tags($item->post_excerpt);// }
						$sub_title = get_field('sub_title', $item->ID);
						if($sub_title){
							$content = substr($sub_title, 0, strpos($sub_title, ' ', 100));
							
							if(strlen($content)<20){
								$content = substr($sub_title, 0, strpos($sub_title, ' ', 31));
							}
						}else{
							$content = strip_tags($item->post_content);
							$content = substr($content, 0, strpos($content, ' ', 100));
        ...
        <div class="container">
          <div class="title"><h2><?php echo $category->name; ?></h2><a href="<?php echo get_category_link($category->term_id); ?>"><span class="see-all">see all</span></a><hr></div>
			...
			<div class="col-md-4 col-mobile">
				...
				<div class="text-box">
				  <h5><?php echo $item->post_title; ?></h5>
				  <span class="date 33"><?php echo date('M d, Y', strtotime($item->post_date)); ?></span>
				  <?php 
					//$content=strpos($item->post_content, ' ', 100);
					/*$content = strip_tags($item->post_content);
